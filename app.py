...
if menu == "Precificação":
    ...
    aba1,aba2,aba3 = st.tabs(["Bebidas","Frutas e Insumos","Artesanais"])
...
# -------------------------
    ...
    bebidas = pd.read_sql("SELECT tipo FROM precos_bebidas", conn)
    tab1,tab2,tab3,tab4 = st.tabs(["Entrada","Saída","Estoque físico","Registros"])
    ...
    with tab1:
        with st.form("entrada_estoque",clear_on_submit=True):
            produto = st.selectbox("Tipo", bebidas["tipo"].unique())
            marca = st.text_input("Marca")
            qtd = st.number_input("Quantidade", min_value=0.0)
            status = st.selectbox("Status", ["Compra","Volta evento"])
            ...
            if st.form_submit_button("Registrar entrada"):
                ...
                if atual.empty:
                    cursor.execute("INSERT INTO estoque VALUES(?,?,?)", (produto,marca,qtd))
                else:
...
# =========================
    ...
    local_evento = col2.text_input("Local do Evento")
    cidade_evento = col3.text_input("Cidade")
    data_evento = col4.date_input("Data do Evento")
    ...
    if selecao:
        ...
        df_artesanais = pd.read_sql("SELECT * FROM precos_artesanais", conn)
        ...
        for fruta, qtd_gramas in ingredientes_insumos.items():
            encontrado = df_insumos[df_insumos["nome"].str.lower() == fruta]
            if not encontrado.empty:
                preco_kg = encontrado.iloc[0]["preco"]
                quantidade_kg = encontrado.iloc[0]["quantidade"]
                custo_item = qtd_gramas * (preco_kg / (quantidade_kg * 1000))
        ...
        # -------------------------
        st.subheader("💵 Valor de Venda e Margem")
        margem = st.number_input("Margem de venda (%)", min_value=0.0, max_value=300.0, value=50.0)
        valor_sugerido = custo_total * (1 + margem / 100)
        st.metric("💰 Valor Sugerido de Venda", f"R$ {valor_sugerido:,.2f}")
        ...
        if st.button("💾 Salvar Orçamento"):
            ...
            # Cria tabela de orçamentos se não existir
            cursor.execute("""
            ...
            detalhes = json.dumps({
                "drinks": selecao,
                "pesos": pesos,
                "ingredientes_totais": ingredientes_totais,
                "bebidas": escolhas_marcas
            })

            cursor.execute("""
            INSERT INTO orcamentos (cliente, local, cidade, data, detalhes, custo, valor_sugerido, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                nome_cliente,
                local_evento,
...
# -------------------------
    ...
    with tab1:
        ...
        tipo = st.selectbox("Tipo do serviço", ["Bar Principal","Whisky","Gin","Spritz","Shots","Outro"])
        itens = st.text_area("Itens (um por linha)", placeholder="jack daniels\nred label\ngelo")
        extras = st.text_area("Extras opcionais", placeholder="gelo saborizado\ncopo especial")
        if st.button("💾 Salvar pacote"):
            dados = json.dumps({"itens": itens.split("\n"), "extras": extras.split("\n")})
            cursor.execute("INSERT INTO pacotes (nome, tipo, dados) VALUES (?,?,?)",(nome,tipo,dados))
            conn.commit()
...
# === O restante do código (Orçamentos, Vendas, Pacotes) continua igual, sem erros de indentação, com blocos 'elif' corretos. ===
