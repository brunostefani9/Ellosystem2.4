...
# =========================================================
    ...
    with tab1:
        ...
        # =========================================================
        # ABA 2 - EVENTOS / CHECKLIST
        # =========================================================
        with tab2:
        
            st.subheader("📋 Eventos e Checklists")
            ...
            if df_eventos.empty:
                ...
                for _, row in df_eventos.iterrows():
                    ...
                    if st.session_state[
                        ...
                        # =================================================
                        # FUNÇÃO MONTAR CHECKLIST
                        # =================================================
        
                        def montar_checklist(df, nome_coluna):
                        ...
                        if bebidas.empty:
                            ...
                            df_bebidas_check = montar_checklist(
                                bebidas,
                                "Bebida"
                            )
        
                            df_bebidas_editado = st.data_editor(
                            ...
                            if st.button(
                                "💾 Salvar conferência das bebidas",
                                key=f"salvar_bebidas_{evento_id}"
                            ):
        
                                for indice, item in df_bebidas_editado.iterrows():
        
                                    item_id = df_bebidas_check.iloc[
                                        indice
                                    ]["_id"]
                        ...
                        if frutas.empty:
                            ...
                            df_frutas_check = montar_checklist(
                                frutas,
                                "Fruta"
                            )
        
                            df_frutas_editado = st.data_editor(
                            ...
                            if st.button(
                                "💾 Salvar conferência das frutas",
                                key=f"salvar_frutas_{evento_id}"
                            ):
        
                                for indice, item in df_frutas_editado.iterrows():
        
                                    item_id = df_frutas_check.iloc[
                                        indice
                                    ]["_id"]
                        ...
                        if insumos.empty:
                            ...
                            df_insumos_check = montar_checklist(
                                insumos,
                                "Insumo"
                            )
        
                            df_insumos_editado = st.data_editor(
                            ...
                            if st.button(
                                "💾 Salvar conferência dos insumos",
                                key=f"salvar_insumos_{evento_id}"
                            ):
        
                                for indice, item in df_insumos_editado.iterrows():
        
                                    item_id = df_insumos_check.iloc[
                                        indice
                                    ]["_id"]
    ...
    with tab3:
        ...
        st.caption(
            "Registre e edite valores adicionais cobrados do cliente "
            "e eventuais custos adicionais da equipe."
        )
        ...
        with st.expander(
            ...
            try:
                ...
                if df_eventos_adendo.empty:
                    ...
                    st.info(
                        "Nenhum evento disponível para adendo."
                    )
                    ...
                    with col6:
                        ...
                        data_pagamento = st.date_input(
                            "📅 Data do Pagamento",
                            value=None,
                            key="cmv_data_pagamento"
                        )

                    salvar_adendo = st.button(
                        "💾 Salvar Adendo",
                        key="cmv_salvar_adendo"
                    )
                    ...
                    if salvar_adendo:
                        ...
                        if (
                            ...
                            st.warning(
                                ...
                                "cobrado do cliente ou custo da equipe."
                            )
        ...
        try:
            ...
            if df_adendos_edicao.empty:
                ...
                if "evento" in df_adendos_edicao.columns:
                ...
                if (
                    filtro_evento != "Todos"
                    and
                    "evento" in df_adendos_edicao.columns
                ):
                ...
                # =================================================
                # NORMALIZA TIPOS ANTES DO DATA_EDITOR
                # =================================================
                #
                # IMPORTANTE:
                # O Supabase pode devolver timestamptz como
                # string/objeto. O data_editor exige um tipo
                # compatível com DatetimeColumn.
                #
                # Aqui convertemos explicitamente antes de
                # abrir o editor.
                # =================================================

                if "data_pagamento" in df_edicao.columns:

                    df_edicao["data_pagamento"] = pd.to_datetime(
                        df_edicao["data_pagamento"],
                        errors="coerce",
                        utc=True
                    )

                    # Remove timezone para evitar incompatibilidade
                    # entre valores timezone-aware e DatetimeColumn.
                    df_edicao["data_pagamento"] = (
                        df_edicao[
                            "data_pagamento"
                        ]
                ...
                for coluna in [
                    ...
                    if coluna in df_edicao.columns:

                        df_edicao[coluna] = pd.to_numeric(
                            df_edicao[coluna],
                            errors="coerce"
                        ).fillna(0.0)
                ...
                st.markdown("---")
                st.markdown("### 📊 Resumo dos Adendos")

                total_cliente_adendos = pd.to_numeric(
                    df_edicao.get(
                        "valor_cliente",
                ...
                total_equipe_adendos = pd.to_numeric(
                    df_edicao.get(
                        "valor_equipe",
                        pd.Series(dtype=float)
                    ),
